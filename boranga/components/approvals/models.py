...
#        from boranga.components.approvals.pdf import create_approval_pdf_bytes
#        copied_to_permit = self.copiedToPermit_fields(self.current_proposal) #Get data related to isCopiedToPermit tag

    def generate_renewal_doc(self):
        from boranga.components.approvals.pdf import create_renewal_doc
    ...
    def approval_surrender(self,request,details):
        with transaction.atomic():
            try:
                if not request.user.boranga_organisations.filter(organisation_id = self.applicant_id):
