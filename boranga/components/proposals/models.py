...
from boranga.components.main.models import CommunicationsLogEntry, UserAction, Document, Region, District, Tenure, ApplicationType, Park, Activity, ActivityCategory, AccessType, Trail, Section, Zone, RequiredDocument#, RevisionedMixin
...
class ProposalType(models.Model):
    #name = models.CharField(verbose_name='Application name (eg. boranga, Apiary)', max_length=24)
    #application_type = models.ForeignKey(ApplicationType, related_name='aplication_types')
    description = models.CharField(max_length=256, blank=True, null=True)
    #name = models.CharField(verbose_name='Application name (eg. boranga, Apiary)', max_length=24, choices=application_type_choicelist(), default=application_type_choicelist()[0][0])
...
#                            user.save()
    ...
    def __create_filming_fee_invoice(self, request):
        ...
        from dateutil.relativedelta import relativedelta
        from boranga.components.bookings.models import FilmingFee
        from boranga.components.bookings.utils import create_filming_fee_lines
...
class DistrictProposal(models.Model):
    ...
    district_approval = models.ForeignKey('boranga.DistrictApproval',null=True,blank=True)
    processing_status = models.CharField('Processing Status', max_length=30, choices=PROCESSING_STATUS_CHOICES,
                                         default=PROCESSING_STATUS_CHOICES[0][0])
    assigned_officer = models.ForeignKey(EmailUser, blank=True, null=True, related_name='boranga_district_proposals_assigned', on_delete=models.SET_NULL)
    assigned_approver = models.ForeignKey(EmailUser, blank=True, null=True, related_name='boranga_district_proposals_approvals', on_delete=models.SET_NULL)
    proposed_issuance_approval = JSONField(blank=True, null=True)
    proposed_decline_status = models.BooleanField(default=False)
    ...
    def final_approval(self,request,details):
        from boranga.components.approvals.models import Approval, DistrictApproval
        with transaction.atomic():
            try:
                if not self.can_assess(request.user):
                    raise exceptions.ProposalNotAuthorized()
...
reversion.register(ReferralRecipientGroup, follow=['boranga_referral_groups', 'referral_assessment'])
